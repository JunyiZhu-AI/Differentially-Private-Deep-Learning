...
args.clipping, args.subspace_momentum = combinations[args.process]
...
if not os.path.isfile(os.path.join(path, 'res.csv')):
    ...
    df = pd.DataFrame({'best_acc': best_acc, 'clip': args.clipping, 'momentum': args.subspace_momentum}, index=[0])
    while True:
        try:
